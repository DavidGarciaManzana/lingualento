...
function TextForm({text, setText, beforeModal, nextModal, label = null}) {
    ...
    return (
        <form className={styles.textForm} action="" onSubmit={() => {
            ...
            {label && <label className={styles.optionalLabel} htmlFor='text-area'>{label}</label>}
            ...
            <button className={styles.getStartButton} disabled={minLengthValidation() !== ''} type={"submit"}>{t?.textFormNextButton}
            </button>
        </form>
    )
}
